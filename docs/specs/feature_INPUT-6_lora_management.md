...
3. LoRA一覧表示時、お気に入りを先頭にグループ表示し、それ以外はアルファベット順に続く（記号 `-` / `_` は無視して比較）。
...
- [x] UIラベルを i18n 辞書に追加（上記 Labels 表の全項目）
- [x] JA / EN 往復切替（JA→EN→JA）で表示が崩れないことを確認
...
- [x] JA / EN 言語切替で表示が崩れない
- [x] スマホ（モバイルUI）で正常に表示・操作できる
- [x] 既存のLoRAスロット・セッション保存が壊れない
- [x] ログに秘匿情報が出力されない
