...
- 生成ボタン列の `＋ キューに追加` で、現在の入力内容をキューへ積みます。
- `▶ 生成開始` は最優先操作ボタンです。キュー実行中（`state: running`）は無効化されます。
- キュー操作:
  - `一時停止`: 実行中ジョブを最後まで完了させた後に停止します（即時中断ではありません）。
  - `再開`: `queued` ジョブがあれば処理を再開します。
  - `完了をクリア`: `done / failed` の履歴行を整理します。
- 行内操作:
  - `↺`: `done / failed` ジョブを再投入
  - `✕`: 対象ジョブを削除
- 進捗表示:
  - 起動直後にComfyUI進捗イベントが遅延する環境でも、フォールバック進捗（%）を表示します。
- 履歴反映:
  - キュー完了画像は全履歴DBだけでなく、セッション履歴側にも同期表示されます。
- 互換性:
  - フロントが `snapshot` 形式で送る前提ですが、バックエンドでも互換受け口を持ちます。
...
| **v1.5.13** | INPUT-6初期実装: LoRA検索・お気に入り・推奨weight反映・カード割当の基本フローを追加。 |
