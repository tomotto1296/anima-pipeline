...
## 直近追記（v1.5.21-queue-hotfix）
- GEN-2: 生成開始ボタンの優先度を上げ、`▶ 生成開始 ※キュー実行中はdisabled` を明示。
- GEN-2: キューパネルUIを日本語統一（`生成キュー` / `状態:`）し、行高・視認性を改善。
- GEN-2: `↺ / ✕` を小さめアウトライン操作に変更。完了ジョブは薄色表示で区別。
- GEN-2: キュー追加時のPOST形式を修正（`snapshot` 送信）し、履歴の `prompt` が空になる不具合を修正。
- GEN-2: バックエンド側でもフラットpayloadを受けられる保険を追加（Error Handling補強）。
- GEN-2: 起動直後の進捗表示で `/progress` 未反映時にフォールバック進捗を出し、`%` が止まる見え方を緩和。
- GEN-2: 完了画像のセッション履歴同期を強化（`/history_list` 取り込み）。
- 仕様確認: `一時停止` は「実行中ジョブ完了後に停止（即時中断ではない）」で統一。
