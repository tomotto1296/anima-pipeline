...
| ☑ | UI-5: 表示テーマ切替（Device / Light / Dark）とダーク時の視認性最適化 |
| ☑ | GEN-9: 「今日の気分」ボタン（ランダム適用 + STATUSサマリー表示） |
| ☑ | INPUT-1: ランダムキャラプリセット作成（保存確認フロー付き） |
| ☑ | SHARE-1: プリセット共有 zip Export/Import（`/presets_export`・`/presets_import`） |
| ☑ | OUTPUT-9: 履歴モーダルで前回との差分表示（Positiveは常時表示可、Negativeは差分時のみ表示、Show/Hideトグル対応） |
| ☑ | LoRA注入（カードグリッドUI・サムネイル表示・最大4スロット） |
| ☑ | ワークフロー切替（`workflows/` から選択・Node ID 自動検出） |
| ☑ | UI言語切替（日本語 / English） |
| ☑ | 表示テーマ切替（Device / Light / Dark）とダーク時の視認性最適化（選択状態・無効ボタン・入力背景） |
...
- `v1.5.1`: 日常利用強化フェーズ（完了: LoRA管理強化・ランダムプリセット・プリセット共有）
- `v1.5.2`: 量産・目玉フェーズ（予定: 一括生成・生成キュー・比較生成・画像→プロンプト再現）
